...
### 4.变量初始化
&emsp;&emsp;没有初始化的数组会被指向null，引用null会抛异常。  
&emsp;&emsp;没有初始化的局部变量会导致编译错误。  
&emsp;&emsp;没有初始化的成员变量会被自动初始化。  
### 5.Static关键字
&emsp;&emsp;使用Static的两种原因：  
1. 只想为特定域分配单一存储空间
...
## 一、初始化
### 1.构造器初始化顺序
&emsp;&emsp;在类的内部，变量定义的先后顺序决定了初始化的顺序，但他们依旧会在任何方法被调用之前得到初始化。  
&emsp;&emsp;先初始化静态对象，然后才是非静态对象。  
&emsp;&emsp;如果类没有定义`toString`方法，`print`将会打印"类名@对象地址"
## 二、垃圾清理
### 1.GC技术原理
1. 引用计数  
&emsp;&emsp;每个对象都有一个计数器，当有引用连接至对象时，引用计数+1，当引用离开作用域或者被置为null时，引用计数-1。  
&emsp;&emsp;引用计数通常在数值为0时立即回收，但是如果对象间存在循环引用，则可能会出现“应该被回收，但计数不为0”的情况。   
&emsp;&emsp;(这种方法常用来说明GC的工作方式，但是没有JVM使用这种方式)  
2. 停止-复制  
&emsp;&emsp;暂停程序运行，将所有存活对象从当前堆复制到另一个堆，没有被复制的就全部是垃圾，当复制到新堆时，变量排列紧凑，所以可以直接分配新空间。  
&emsp;&emsp;“按需从堆中分配几块较大的内存，复制动作发生在内存之间“
3. 标记-清扫  
&emsp;&emsp;从堆栈和静态区出发，遍历所有的引用，并对对象进行标记，在全部标记工作完成后，将没有标记的对象回收。  
&emsp;&emsp;同样也需要暂停程序。

# 第六章&nbsp;&nbsp;访问权限控制
## 一、访问权限
### 1. 包访问权限
&emsp;&emsp;没有设置访问权限的默认为包访问权限，包访问权限中同一包内的所有类成员可互相访问，但对包外的类而言则是Private的。  
&emsp;&emsp;如果两个类处在同一目录下，并且没有设置任何包名称，Java会认为这些类处于该目录下的默认包中。
### 2.Public
&emsp;&emsp;一个编译单元只能有一个Public类，并且类名需要与文件名完全一致。
### 3.Private
&emsp;&emsp;用Private修饰构造器，在其他方法中return一个新对象，这样可以阻止对构造器的直接访问。
### 4.Protected
&emsp;&emsp;被Protected修饰的成员可被派生类访问，同时还提供包访问权限。
# 第七章&nbsp;&nbsp;复用类（继承）
### 1. 初始化
&emsp;&emsp;总是先初始化基类，再初始化派生类，即使没有基类对象，也要初始化基类。  
&emsp;&emsp;基类构造器总是会被调用。  
&emsp;&emsp;如要调用带参的基类构造器，需要调用`super()`
### 2. @Override
&emsp;&emsp;该注解没有什么额外功能，只是说明该方法要进行Override，如果使用了该注解但没有进行Override，会触发编译错误。
### 3.组合与继承
1. 组合用于想在新类中使用现有类的功能。
2. 继承用于使用某个现有类，并基于它开发一个特殊版本。

### 4.Final
&emsp;&emsp;用Final定义的基础变量类型无法被改变。  
&emsp;&emsp;用Final定义的对象无法改变其引用指向，但是可以修改它引用的对象的值。  
&emsp;&emsp;可以创建不赋值的空白Final，但是必须保证使用前被初始化。  
&emsp;&emsp;方法中被Final修饰的参数只能读不能写。   
&emsp;&emsp;用Final修饰的类无法更改，无法继承。   
&emsp;&emsp;Final的意义：把方法锁定，以防任何继承类修改。  
# 第八章&nbsp;&nbsp;多态
### 1.基类与派生类
1. 基类为其所有派生类建立公共接口，但其派生类向上转型由基类引用后，仍可找到其override的接口。  
&emsp;&emsp;但是只有非Private方法可以被覆盖，因为Private对派生类不可见，派生类中的方法相当于新方法。  
2. 派生类的构造器必然会调用基类的构造器，如果没有显式调用，则会自动调用基类默认构造器，如果没有默认构造器则会编译错误。  
&emsp;&emsp;如果需要手动清理对象，则必须在派生类中覆盖清理方法，并调用基类的清理方法，销毁顺序与初始化顺序相反。
...
### 3.协变返回类型
&emsp;&emsp;在派生类中被覆盖的方法可以返回基类的返回类型的某种派生类型。

# 第九章&nbsp;&nbsp;接口
### 1.Abstract抽象类
&emsp;&emsp;如果一个类包含一个或多个抽象方法，则必须被限定为抽象类，但抽象类可以没有抽象方法。  
&emsp;&emsp;抽象类无法产生对象，继承自抽象类的类如果没有完全实现其抽象方法，则也必须用abstract修饰。 
### 2.Interface接口
&emsp;&emsp;用Interface修饰的接口完全由抽象方法组成，不提供任何实现。  
&emsp;&emsp;接口中的任何域都自动是static和final的。  
&emsp;&emsp;接口中的方法默认为Public，所以实现接口时也必须定义为Public。  
### 3.多重继承
&emsp;&emsp;Java是单继承，但是一个类可以继承自一个基类，同时实现多个接口。
### 4.接口扩展
&emsp;&emsp;接口可以继承，通过继承可以在接口中添加新方法，从而扩展接口。
# 第十章&nbsp;&nbsp;内部类
### 1.内部类
&emsp;&emsp;在类的内部定义另一个类，内部类拥有其外围类的所有元素的访问权。
### 2.this与new
&emsp;&emsp;用`OuterClass.this`可以获得一个外部类对象的引用。  
&emsp;&emsp;用`OuterClassObject.new`可以使其创建一个内部类对象。
### 3.匿名内部类
```Java
...
```
### 4.嵌套类
&emsp;&emsp;将内部类用static修饰
1. 要创建嵌套类的对象，并不需要其外围类的对象。
2. 不能从嵌套的对象中访问非静态的外围对象。

&emsp;&emsp;借助嵌套类，可以在接口中实现一些代码，甚至可以实现外围接口。
# 第十一章&nbsp;&nbsp;持有对象
## 一、容器类
1. Collection：
    List/Set/Queue
...
## 二、迭代器
### 1.Iterator
&emsp;&emsp;使用iterator()方法返回一个Iterator，包含`next()/hasNext()/remove()`方法。  
&emsp;&emsp;迭代器不关心容器的类型，只关心容器中的变量类型。
### 2.ListIterator
&emsp;&emsp;只能用于List容器，但是可以双向移动。  
&emsp;&emsp;包含`next()/previous()/set()`等方法
### 三、Set
&emsp;&emsp;Set就是Collection的一种实现，接口完全一样。
1. HashSet使用散列结构
2. TreeSet使用红黑树
3. LinkedHashList使用散列，但看起来像是使用了链表。
4. TreeSet中保存的内容是有序的

&emsp;&emsp;用`contains()`可以判断Set是否包含元素。
### 四、Map
&emsp;&emsp;将容器组合起来可以获得多为扩展，例如`Map<Int,List<>>`。
### 五、Queue
&emsp;&emsp;PriorityQueue可以按优先级排序，通过构建Comparator来实现优先级比较。

# 第十二章&nbsp;&nbsp;异常处理
&emsp;&emsp;用Try包裹可能产生异常的语句块，用Catch进行捕获。  
&emsp;&emsp;仅搜索第一个匹配的Catch块，Catch执行后即认为异常得到处理，不会再匹配其他符合要求的Catch，即便再次抛出，也只会抛给上层处理，不会再被Catch捕获。
...
### 2.自定义异常
&emsp;&emsp;可以通过继承自异常类来创建自定义类型的异常。
### 3.Finally
&emsp;&emsp;由于Java不需要进行内存回收，也不需要析构，所以Finally中主要负责将除内存之外的其他资源恢复到初始状态。
# 第十四章&nbsp;&nbsp;类型信息
## 一、ClassLoader
&emsp;&emsp;Java的Class是动态加载的，各个部分在必需时才会被加载。   
&emsp;&emsp;先检查类是否加载，若尚未加载，默认的类加载器根据类名查找Class。  
&emsp;&emsp;在加载时会验证类有没有被破坏，并且是否包含不良代码。  
...
&emsp;&emsp;`Filelock.release()`可以释放锁。  
&emsp;&emsp;trylock或lock可以带参数来指定加锁范围，只会锁定固定区域，不带参的锁将根据文件尺寸变化而变化。
### 2.锁的类型
1. 独占锁：锁定的资源只允许加锁的程序使用。
2. 共享锁：锁定的资源可被其他程序读取，但不能更改。

&emsp;&emsp;锁的类型由操作系统底层提供，如果不支持共享锁，调用时会返回独占锁。  
&emsp;&emsp;锁的类型可以通过`Filelock.isShared()`查询。
### 3.对象序列化
&emsp;&emsp;对于实现了Seriallized接口的对象，可以使用writeObject将其序列化，序列化可以将对象的关系网全部保存，可以借助readObject读取。  
...
### 4.控制序列化
&emsp;&emsp;让类实现接口Externalizable，并实现writeExternal和readExternal方法，但该类的默认构造器需为public，因为所有的默认构造器都会被调用。
### 5.Transient
&emsp;&emsp;被transient修饰的变量在被序列化时不会被处理，可以防止敏感部分被序列化。
# 第二十章&nbsp;&nbsp;注解
## 一、定义注解
```Java
//定义注解用在什么地方
...
```
## 二、实现注解处理器
&emsp;&emsp;通过`method.getAnnotation(ClassName.class)`来判断方法上是否有指定类型的注解，并进行后续处理。
# 第二十一章&nbsp;&nbsp;并发
## 一、任务定义与执行
### 1.定义任务
&emsp;&emsp;实现Runable接口并编写run()方法，并在run中实现所需业务操作。
### 2.Thread类
```Java
Thread t=new Thread(new ClassA());
t.start();
```
&emsp;&emsp;为Thread构造器传入一个Runable对象，再调用Thread的`start()`方法，此时会创建一个新线程。
### 3.Executor
```Java
...
exec.shutdown();
```
&emsp;&emsp;通过`exec.execute`提交任务，`exec.shutdown`可以防止新任务被提交，但是已提交的任务会继续执行，将在所有任务完成后退出。
```Java
...
## 二、任务回调
### 1.可回调的任务
&emsp;&emsp;Runable只可执行，不能回调。  
&emsp;&emsp;需要实现Callable接口，类型参数为call()方法的返回值。
### 2.任务调用
&emsp;&emsp;须用`exec.submit(new ClassA())`来调用，submit方法会返回Future对象，Future是泛型的，其参数类型是回调函数返回值的类型。  
&emsp;&emsp;可以用isDone()方法来查询Future是否完成，任务完成时，会产生结果，可用get()方法进行获取，如果在未完成时即调用get()方法，则会一致阻塞到有结果产生。
## 三、任务控制
### 1.休眠
```Java
Thread.sleep()                  (老方式)  
...
```
### 2.优先级
&emsp;&emsp;在任务内部，调用`Thread.currentThread().getPriority()`来获取优先级。调用`Thread.currentThread().setPriority()`来修改优先级。
...
### 3.让步
&emsp;&emsp;使用yield方法可以暗示建议其他具有相同优先级的线程先运行，但是不保证效果。
### 4.后台线程
&emsp;&emsp;在`Thread.start()`之前先`Thread.setDaemo(true)`将线程设置为后台线程，在程序的非后台线程结束时，程序会中指运行，并杀死所有的后台线程。
### 5.异常处理
&emsp;&emsp;无法直接用try-catch从线程中捕获异常，但是可以在每一个Thread对象上附着一个异常处理器。
```Java
Thread t=new Thread();
...
## 四、资源共享
### 1.加锁
&emsp;&emsp;用`synchronized`修饰的方法或代码片段，会在执行时检查->获取锁->执行->释放锁。  
&emsp;&emsp;一个对象中的所有同步方法共享一个锁，若某个任务调用了其中一个加锁方法，则只有等其结束并释放后，其他任务才能调用其中的任何一个同步方法。
### 2.原子性
...
&emsp;&emsp;高级封装：管道(PipedWriter/PipedReader)。
