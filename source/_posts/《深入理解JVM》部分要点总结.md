...
&emsp;&emsp;不需要连续的内存，可以选择固定大小或可扩展的大小，此外还可以选择不实现GC。   
&emsp;&emsp;该区域的GC目标主要是对常量池的回收和对类型的卸载。  
&emsp;&emsp;当方法区无法满足内存分配需求时，会抛出OutOfMemoryError异常。
### 2.虚拟机栈
&emsp;&emsp;虚拟机栈是线程私有的，主要执行Java方法，描述了Java方法执行的内存模型。  
&emsp;&emsp;每个方法在执行时都会创建一个栈帧，用于存储局部变量表、操作数栈、动态链接、方法出口等。一个方法从调用至执行完成的过程，就对应着一个栈帧在VM Stack中从入栈到出栈的过程。

&emsp;&emsp;局部变量表：存放了编译器可知的基本数据类型、对象引用、returnAddress。该表所需的内存空间在编译器就会完成分配，运行期间不会改变大小。

&emsp;&emsp;当线程请求的栈深度超出了JVM允许的深度，会抛出StackOverflowError异常，当扩展时无法申请到足够的内存时，会抛出OutOfMemoryError异常。

### 3.本地方法栈
&emsp;&emsp;本地方法栈也是线程私有的，主要执行虚拟机用的Native方法。  
&emsp;&emsp;在HotSpot中，本地方法栈与虚拟机栈合二为一。
### 4.堆
&emsp;&emsp;堆是所有线程共享的，是JVM所管理的内存中最大的一块，几乎所有的数组/对象实例都在这里分配内存。

&emsp;&emsp;Java堆可以细分为新生代和老年代，再细分可以分为Eden、From Survivor、To Survivor空间。

&emsp;&emsp;Java堆在物理上可以是不连续的，但在逻辑上是连续的，当内存不足以完成实例分配且无法扩展时，会抛出OutOfMemoryError异常。
### 5.程序计数器
&emsp;&emsp;程序计数器是线程私有的，是一块较小的内存空间，可看作是当前线程所执行的字节码的行号指示器。每个线程都有一个独立的程序计数器，各计数器互不影响。

1. 执行Java方法：计数器记录正在执行的虚拟机字节码指令地址
2. 执行Native方法：计数器值为空

### 6.直接内存
&emsp;&emsp;NIO提供了一种基于通道和缓冲区的I/O方式，可以使用Native函数库直接分配堆外内存，然后通过一个存在堆中的DirectByteBuffer对象作为这块内存的引用。  
这种方法可以避免在Java堆和Native堆中来回复制从而提高性能。

## 二、创建对象
&emsp;&emsp;在遇到New指令后执行如下步骤：
### 1.检查
&emsp;&emsp;检查能否在常量池定位类的符号引用，并检查这个类是否被加载解析、初始化，如果没有，则先加载类，加载完成后执行第二步。
### 2.分配内存
&emsp;&emsp;对象所需的内存大小在类加载完成时即确定，内存分配主要有两种方式：
...
&emsp;&emsp;选择哪种方式来进行内存分配取决于堆是否规整，而堆是否规整取决于GC是否带有压缩整理功能。
...
## 一、Class类文件结构
### 1.前4字节
&emsp;&emsp;魔数，用于确定这个文件是否是一个能够被VM接受的class文件，class文件的前四字节为0xCAFEBABE
### 2.5-8字节
&emsp;&emsp;第5和第6字节为版本号，第7和第8字节为主版本号。  
&emsp;&emsp;高版本JDK只能运行以前的class文件，不能运行更高版本的class文件。
### 3.常量池
&emsp;&emsp;主要存放两大类常量：字面量，符号引用(类和接口的全限定名/字段的名称和描述符/方法的名称和描述符)。  
&emsp;&emsp;入口有一个u2类型的变量，表示池中常量的数量，随后根据常量类型标志+常量内容位来表示常量。
### 4.访问标志
&emsp;&emsp;用两字节表示访问标志，用于标识一些类或接口层次的访问信息。
### 5.类索引，父类索引与接口索引集合
### 6.字段表集合
&emsp;&emsp;用于描述接口或勒种生命的变量，包括类级变量和实例级变量，但不包括方法内部的局部变量。  
&emsp;&emsp;其中两字节用来描述字段访问性，两字节用来表示名称(常量池索引),两字节用来表示描述符(常量池索引)。
### 7.方法表集合
### 8.属性表集合
&emsp;&emsp;部分属性:  
...
&emsp;&emsp;加载:load  
&emsp;&emsp;存储:store  
&emsp;&emsp;常量加载到操作数栈:push  
&emsp;&emsp;扩充局部变量表的访问索引:wide
### 2.运算
&emsp;&emsp;加减乘除:add/sub/mul/div  
&emsp;&emsp;取模:rem  
&emsp;&emsp;位移:shl/shr  
&emsp;&emsp;取反:neg  
&emsp;&emsp;按位或:or  
&emsp;&emsp;按位与:and  
&emsp;&emsp;按位异或:xor  
&emsp;&emsp;自增:inc  
&emsp;&emsp;比较:cmpg,cmpl  
### 3.类型转换
i2b/i2c/i2s/l2i/f2i......
### 4.对象
&emsp;&emsp;对象创建:new  
&emsp;&emsp;数组创建:newarray、anewarray、multianewarray  
&emsp;&emsp;访问类实例:getfield、putfield、getstatic、putstatic  
&emsp;&emsp;数组压栈:aload  
&emsp;&emsp;将栈值存到数组:astore  
&emsp;&emsp;取数组长度:arraylength  
...
### 5.准备
&emsp;&emsp;为类变量分配内存并设置初始值，内存分配仅包括类变量(被static修饰的变量)，初始值也只是赋0值，但被final修饰的变量会赋具体值。
### 6.解析
&emsp;&emsp;将常量池中的符号引用转换为直接引用，主要完成如下解析:
...
### 7.初始化
&emsp;&emsp;`<clinit>`由编译器自动收集类中所有类变量赋值动作和静态语句块合并产生。VM中第一个被执行的`<clinit>()`方法一定是`java.lang.object`。  
&emsp;&emsp;如果各线程同时初始化一个类，只有一个线程会执行`<clinit>()`方法，其他线程都会阻塞。
## 二、类加载器
### 1.类相等判断
...
&emsp;&emsp;应用程序加载器：系统类加载器，也是默认加载器。

&emsp;&emsp;所有请求都会向上传递至启动类加载器，当无法完成加载请求时才会向下传递，再由子加载器进行加载。
...
&emsp;&emsp;slot是可以复用的，这回影响GC的行为，一个变量可能已经无用了，但局部变量表中还保存着该变量的引用，这个变量就不会被回收。当手动设置为null并被其他新变量覆盖时才会GC。
### 2.操作数栈(后入先出)
&emsp;&emsp;初始为空，在方法执行时由字节码指令入/出栈。
### 3.动态连接
&emsp;&emsp;指向运行时常量池中该栈帧所属方法的引用。
### 4.方法返回地址
&emsp;&emsp;保存方法被调用的位置。

# 二、方法调用
### 1.解析
&emsp;&emsp;在解析阶段，会将一部分符号引用转换为直接引用。  
&emsp;&emsp;该部分替换的都是非虚方法，主要包括静态方法、私有方法、实例构造器、父类方法、Final方法。
### 2.分派
